...
const LanguageCard = ({
  ...
  return (
    <div className="relative w-full group">
      {/* Soft ambient aura (subtle, only on hover a bit stronger) */}
      <div className="pointer-events-none absolute -inset-5 rounded-[1.6rem] -z-10 opacity-10 sm:opacity-15 group-hover:opacity-25 transition-opacity duration-500">
        <div
          className="
            absolute inset-0 rounded-[inherit] blur-2xl
            bg-[radial-gradient(120%_80%_at_50%_-20%,rgba(168,85,247,0.22),transparent_60%),radial-gradient(140%_100%_at_100%_120%,rgba(59,130,246,0.18),transparent_60%)]
          "
        />
      </div>
      ...
      <div
        className={`
          ...
          ${active
            ...
            : "border-white/10 group-hover:border-violet-400/30"}
          ${isAction
            ? "cursor-pointer"
            : "cursor-default"}
          shadow-[0_0_0_0_rgba(0,0,0,0)]
          group-hover:shadow-[0_0_24px_rgba(139,92,246,0.20)]
        `}
      ...
      >
        {/* Inner glass sheen (very subtle, no rainbow) */}
        ...
        {/* Title / Country code */}
        <span
          className={`relative z-10 text-white font-semibold text-lg sm:text-xl mb-1 ${isAction ? "flex items-center gap-2" : ""}`}
        >
          {countryCode}
          {isAction && (
